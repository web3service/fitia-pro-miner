/**
 * FITIA PRO MINER — app.js v6 (fixed)
 * ==============================================================
 * Aligned with FitiaMiningV3_Core + FitiaMiningV3_Mine contracts.
 * Dual-contract architecture: Core (balances/swaps) + Mine (machines/mining).
 * Internal balance system: users must deposit USDT/FTA into Core first.
 */
...
de: {
 ...
 deposit:"Einzahlen",depositDesc:"Token intern einzahlen.",depositBtn:"EINZAHLEN",
 withdraw:"Abheben",withdrawDesc:"Token abheben.",withdrawBtn:"ABHEBEN",
 ...
 claiming:"Einforderung...",sending:"Senden...",depositing:"Einzahlung...",withdrawing:"Abhebung...",
 ...
 profileUpdated:"Profil aktualisiert!",deposited:"Eingezahlt!",withdrawn:"Abgehoben!",
 ...
 errApprovalFailed:"Genehmigung fehlgeschlagen.",errNoBal:"Internes Guthaben zu niedrig.",
 useUsdtInstead:"USDT verwenden.",
},
zh: {
...
sg: {
 ...
 noActiveMachines:"No active machines",
 deposit:"Deposit",depositDesc:"Deposit tokens to internal balance.",depositBtn:"DEPOSIT",
 withdraw:"Withdraw",withdrawDesc:"Withdraw tokens to wallet.",withdrawBtn:"WITHDRAW",
 ...
 profileUpdated:"Profile updated!",deposited:"Deposited!",withdrawn:"Withdrawn!",
 ...
 errApprovalFailed:"Approval failed.",errNoBal:"Insufficient internal balance. Deposit first.",
 useUsdtInstead:"Try USDT.",
}
};

/* ═════════════════════════════════════════════════════════════
 CHAT INTENTS + RESPONSES  (unchanged — abbreviated for space)
 ═════════════════════════════════════════════════════════════ */
const CHAT_INTENTS = {
 ...
 deposit_help:{weight:4,keywords:{all:['how to deposit','deposit usdt','deposit fta','déposer','存入','where is my balance','internal balance']}}
};
...
const CHAT_RESPONSES = {
 ...
 how_mining_works:{en:`⛏️ *How Mining Works:*\n\n1️⃣ Deposit USDT to Core (Wallet tab)\n2️⃣ Buy a Machine (Shop tab, USDT recommended)\n3️⃣ Buy a Battery (3-365 days)\n4️⃣ Plug it In (Wallet tab)\n5️⃣ Earn FTA every second\n6️⃣ Claim rewards (gold CLAIM button)\n\n⚡ Higher tier = more power | 🔋 Longer battery = better value/day`},
 how_swap_works:{en:`💱 *Swap USDT↔FTA*\n\n🔹 Deposit USDT or FTA to Core first (Wallet tab)\n🔹 USDT→FTA: Pay USDT, get FTA. Builds protocol liquidity!\n🔹 FTA→USDT: Sell FTA, get USDT. Needs liquidity > 0.\n\n📊 Protocol uses bonding curve — price adjusts with supply/demand.\n⚠️ If liquidity is 0, swap USDT→FTA first or use USDT to buy machines.`},
 beginner_guide:{en:`🚀 *Getting Started*\n\n📱 1. Install MetaMask → Polygon network\n💰 2. Buy POL + USDT → send to MetaMask\n🔗 3. Connect wallet → Deposit USDT to Core\n⛏️ 4. Shop → buy Starter MK-I (USDT)\n🔋 5. Buy battery → Wallet → Plug in\n🎉 DONE! Mining FTA every second.`},
 ...
 deposit_help:{en:`💰 *How to Deposit*\n\n1. Open Wallet tab → find Deposit section\n2. Choose USDT or FTA\n3. Enter amount → tap DEPOSIT\n4. First time: approve the Core contract\n5. Tokens move to your INTERNAL balance\n\nYour internal balance is used for purchases & swaps. Withdraw anytime!`},
 default:{en:`🤔 I'm the Fitia Assistant — ask me about: Mining, Swaps, Deposits, the 4 Visions, Getting Started, Security, or anything Fitia!`}
};

/* ═════════════════════════════════════════════════════════════
 ABIs — matched to actual contract function signatures
 ═════════════════════════════════════════════════════════════ */
...
const CORE_ABI = [
 ...
 "function withdrawPol(uint256 a) external",

 // Referral
 "function setReferrer(address r) external",
 "function setReferrerById(uint256 rid) external",
...
const MINE_ABI = [
 ...
 "function addMachineType(uint256 p, uint256 pw) external",
 "function setMachinePrice(uint256 id, uint256 p) external",

 // Battery types
 "function bCount() view returns (uint256)",
 "function bTypes(uint256) view returns (uint256 price, uint256 dur)",
 "function addBatteryType(uint256 p, uint256 d) external",
 "function setBatteryPrice(uint256 id, uint256 p) external",
...
const SWAP_DEADLINE_SEC = 1200; // 20 min
...
/* ═════════════════════════════════════════════════════════════
 APPLICATION  (fixed — dual-contract, internal balances)
 ═════════════════════════════════════════════════════════════ */
class Application {
 constructor(){
 this.provider = null;
 ...
 this.usdt = null;       // ERC20 USDT
 this.fta = null;        // ERC20 FTA
 this.user = null;
 this.payMode = 'USDT';
 ...
 this.chatHistory = [];
 this.netFtaSold = 0n;
 this.internalU = 0n;    // user's uBal on Core
 this.internalF = 0n;    // user's fBal on Core
 ...
 walletAddress: this.user, signature, challenge: challengeRes.challenge
 });
 ...
 const f = { en:'🇬🇧', fr:'🇫🇷', de:'🇩🇪', zh:'🇨🇳', sg:'🇸🇬' };
 document.getElementById('lang-btn-display').innerText = `${f[lang]} ${lang.toUpperCase()}`;
 this.applyTranslations();
 this.renderShop();
 }
 applyTranslations(){
 const st = (s,k) => { const e = document.querySelector(s); if (e) e.innerText = this.t(k); };
 st('#btn-connect','connect');
 st('.total-balance-card small','totalBal');
 ...
 document.querySelectorAll('.nav-item span').forEach((s,i) =>
 s.innerText = this.t(['home','shop','assets','swapNav','historyNav'][i])
 );
 ...
 // Auth + History (background)
 setTimeout(() => this.authenticate(), 500);
 setTimeout(() => this.refreshHistory(), 2000);
 this._historyInterval = setInterval(() => this.refreshHistory(), 60000);
 }
 ...
 try {
 const cnt = Number(await this.mine.bCount());
 for (let i = 0; i < cnt; i++) {
 try {
 ...
 async refreshInternalBalances(){
 if (!this.user || !this.core) return;
 try {
 this.internalU = await this.core.uBal(this.user);
 this.internalF = await this.core.fBal(this.user);
 } catch(e) {
 console.warn('refreshInternalBalances', e);
 this.internalU = 0n;
 this.internalF = 0n;
 }
 }
 ...
 // Approve Core to spend USDT
 const allowance = await this.usdt.allowance(this.user, CONFIG.CORE);
 if (allowance < raw) {
 this.setLoader(true, this.t('approveUsdt'));
 const atx = await this.usdt.approve(CONFIG.CORE, raw);
 await atx.wait();
 }
 // Deposit to Core
 this.setLoader(true, this.t('confirming'));
 const tx = await this.core.depositUsdt(raw);
 await tx.wait();
 this.recordTransaction('deposit', 'USDT', '', amount, '', tx.hash, {});

 } else if (tokenSymbol === 'FTA') {
 const raw = ethers.parseUnits(amountStr, this.ftaDecimals);
 // Approve Core to spend FTA
 const allowance = await this.fta.allowance(this.user, CONFIG.CORE);
 if (allowance < raw) {
 this.setLoader(true, this.t('approveFta'));
 const atx = await this.fta.approve(CONFIG.CORE, raw);
 await atx.wait();
 }
 // Deposit to Core
 this.setLoader(true, this.t('confirming'));
 const tx = await this.core.depositFta(raw);
 await tx.wait();
 this.recordTransaction('deposit', 'FTA', '', amount, '', tx.hash, {});

 } else if (tokenSymbol === 'POL') {
 const tx = await this.core.depositPol({ value: ethers.parseEther(amountStr) });
 await tx.wait();
 this.recordTransaction('deposit', 'POL', '', amount, '', tx.hash, {});
 }

 await this.refreshInternalBalances();
 this.showToast(this.t('deposited'));
 document.getElementById('deposit-amount-usdt').value = '';
 document.getElementById('deposit-amount-fta').value = '';
 await this.updateData();
 ...
 let tx;
 if (tokenSymbol === 'USDT') {
 tx = await this.core.withdrawUsdt(ethers.parseUnits(amountStr, this.usdtDecimals));
 } else if (tokenSymbol === 'FTA') {
 tx = await this.core.withdrawFta(ethers.parseUnits(amountStr, this.ftaDecimals));
 } else if (tokenSymbol === 'POL') {
 tx = await this.core.withdrawPol(ethers.parseEther(amountStr));
 }
 await tx.wait();
 await this.refreshInternalBalances();
 this.showToast(this.t('withdrawn'));
 this.recordTransaction('withdraw', tokenSymbol, '', amount, '', tx.hash, {});
 await this.updateData();
 ...
 try {
 // Internal balances from Core
 this.internalU = await this.core.uBal(this.user);
 this.internalF = await this.core.fBal(this.user);

 // Difficulty from Core
 try { this.currentDifficulty = await this.core.difficulty(); } catch(e) {}

 // netFta from Core
 try { this.netFtaSold = await this.core.netFta(); } catch(e) {}

 // Active power from Mine
 try { this.currentRealPower = Number(await this.mine.powerOf(this.user)); } catch(e) {}

 // User machines from Mine (on-chain)
 try {
 const machines = await this.mine.myMachines(this.user);
 this.userMachines = machines.map(m => ({
 typeId: Number(m.tid),
 expiresAt: Number(m.exp),
 pluggedBatteryType: null // derived from exp
 }));
 // Also read user last claim time
 try {
 const info = await this.mine.myInfo(this.user);
 this.userLastClaimTime = Number(info.lc);
 } catch(e) {
 this.userLastClaimTime = Math.floor(Date.now() / 1000);
 }
 } catch(e) {
 console.warn('read machines', e);
 this.userMachines = [];
 this.userLastClaimTime = Math.floor(Date.now() / 1000);
 }

 // User batteries from Mine (on-chain)
 try {
 this.userBatteries = {};
 const bCnt = Number(await this.mine.bCount());
 for (let i = 0; i < bCnt; i++) {
 try {
 const bal = Number(await this.mine.myBattery(this.user, i));
 if (bal > 0) this.userBatteries[i] = bal;
 } catch(e) {}
 }
 ...
 const elapsed = Math.max(0, now - this.userLastClaimTime);
 ...
 // Internal balances (human-readable)
 ...
 // FTA price: sell 1 FTA to get USDT amount
 try {
 const oneFta = ethers.parseUnits("1", this.ftaDecimals);
 const usdtOut = await this.core.sellFta(oneFta);
 this.ftaPriceUsd = parseFloat(ethers.formatUnits(usdtOut, this.usdtDecimals));
 } catch(e) {
 // Fallback: try rate()
 try {
 const r = await this.core.rate();
 this.ftaPriceUsd = parseFloat(ethers.formatUnits(r, this.ftaDecimals));
 } catch(e2) { this.ftaPriceUsd = 0; }
 }
 ...
 document.getElementById('swap-bal-from').innerText =
 parseFloat(ethers.formatUnits(
 this.swapDirection === 'USDT_TO_FTA' ? this.internalU : this.internalF,
 fromDec
 )).toFixed(4);
 document.getElementById('swap-bal-to').innerText =
 parseFloat(ethers.formatUnits(
 this.swapDirection === 'USDT_TO_FTA' ? this.internalF : this.internalU,
 toDec
 )).toFixed(4);
 ...
 try {
 const cnt = Number(await this.mine.mCount());
 const p = [];
 for (let i = 0; i < cnt; i++) p.push(this.mine.mTypes(i));
 ...
 const cnt = Number(await this.mine.bCount());
 const p = [];
 for (let i = 0; i < cnt; i++) p.push(this.mine.bTypes(i));
 const r = await Promise.all(p);
 this.shopBatteriesData = [];
 for (let i = 0; i < cnt; i++) {
 const d = r[i];
 ...
 /* ═══════════ BUY MACHINE (UPDATED — uses internal balances) ══ */
 async buyMachine(id){
 if (!this.user) return this.connect();
 const m = this.shopMachinesData[id];
 const label = `${this.t('buyingMachine')} (${this.payMode})...`;
 this.setLoader(true, label);
 ...
 if (this.internalU < m.priceRaw) {
 this.showToast(`${this.t('errNoBal')}\nNeed: ${m.price.toFixed(2)} USDT\nHave: ${parseFloat(ethers.formatUnits(this.internalU, this.usdtDecimals)).toFixed(2)} USDT`, true);
 this.setLoader(false);
 return;
 }
 tx = await this.mine.buyMachine(id);
 await tx.wait();
 this.recordTransaction('buy_machine', 'USDT', '', m.price, '', tx.hash, { machineTypeId: id, machineTier: id+1 });

 } else {
 // FTA path: calculate via costFta on Core
 ...
 // Get FTA cost for the USDT price
 let ftCost;
 try {
 ftCost = await this.core.costFta(m.priceRaw);
 } catch(viewErr) {
 ...
 // Read fee config from Core
 let tfp = 0;
 try {
 const devFee = Number(await this.core.devFee());
 const cr0 = Number(await this.core.comRates(0));
 const cr1 = Number(await this.core.comRates(1));
 const cr2 = Number(await this.core.comRates(2));
 tfp = devFee + cr0 + cr1 + cr2;
 } catch(e) { tfp = 11; } // default fallback: 5+3+2+1=11

 // Total FTA needed = ftCost + fee portion
 const totalFta = tfp >= 100 ? ftCost : ftCost + (ftCost * BigInt(tfp)) / BigInt(100 - tfp);

 // Check internal FTA balance
 if (this.internalF < totalFta) {
 const need = parseFloat(ethers.formatUnits(totalFta, this.ftaDecimals));
 const have = parseFloat(ethers.formatUnits(this.internalF, this.ftaDecimals));
 this.showToast(`${this.t('errNoBal')}\nNeed: ${need.toFixed(4)} FTA\nHave: ${have.toFixed(4)} FTA\n→ Deposit FTA first!`, true);
 this.setLoader(false);
 return;
 }

 tx = await this.mine.buyMachineFTA(id);
 await tx.wait();
 ...
 /* ═══════════ BUY BATTERY (UPDATED — uses internal balances) ══ */
 async buyBattery(id){
 if (!this.user) return this.connect();
 ...
 if (this.payMode === 'USDT') {
 if (this.internalU < b.priceRaw) {
 this.showToast(`${this.t('errNoBal')}\nNeed: ${b.price.toFixed(2)} USDT\nHave: ${parseFloat(ethers.formatUnits(this.internalU, this.usdtDecimals)).toFixed(2)} USDT`, true);
 this.setLoader(false);
 return;
 }
 tx = await this.mine.buyBattery(id);
 await tx.wait();
 ...
 let ftCost;
 try { ftCost = await this.core.costFta(b.priceRaw); }
 catch(viewErr) { this.showToast(this.t('errLowLiquidity'), true); this.setLoader(false); return; }

 let tfp = 0;
 try {
 tfp = Number(await this.core.devFee()) + Number(await this.core.comRates(0))
 + Number(await this.core.comRates(1)) + Number(await this.core.comRates(2));
 } catch(e) { tfp = 11; }

 const totalFta = tfp >= 100 ? ftCost : ftCost + (ftCost * BigInt(tfp)) / BigInt(100 - tfp);

 if (this.internalF < totalFta) {
 const needF = parseFloat(ethers.formatUnits(totalFta, this.ftaDecimals));
 const haveF = parseFloat(ethers.formatUnits(this.internalF, this.ftaDecimals));
 this.showToast(`${this.t('errNoBal')}\nNeed: ${needF.toFixed(4)} FTA\nHave: ${haveF.toFixed(4)} FTA\n→ Deposit FTA first!`, true);
 this.setLoader(false);
 return;
 }

 tx = await this.mine.buyBatteryFTA(id);
 await tx.wait();
 const ftExact = parseFloat(ethers.formatUnits(ftCost, this.ftaDecimals));
 ...
 const battBal = this.userBatteries[bT] || 0;
 if (battBal <= 0) return this.showToast("No battery of this type available", true);

 this.setLoader(true, this.t('pluggingIn'));
 try {
 const tx = await this.mine.plugInMachine(idx, bT);
 await tx.wait();

 this.recordTransaction('plug_in', '', '', '', '', tx.hash, {
 machineIndex: idx,
 batteryTypeId: Number(bT),
 duration: this.getBatteryDuration(Number(bT))
 });
 ...
 this.recordTransaction('claim', '', 'FTA', '', pending, tx.hash, { amountFTA: pending });
 await this.updateData();
 ...
 /* ═══════════ SWAP (UPDATED — correct signatures, internal balances) ══ */
 toggleSwap(){
 ...
 if (isUsdtTo) {
 // USDT → FTA: use buyFta on Core
 const rawIn = ethers.parseUnits(netInput.toString(), this.usdtDecimals);
 const rawOut = await this.core.buyFta(rawIn);
 netOutput = parseFloat(ethers.formatUnits(rawOut, this.ftaDecimals));
 } else {
 // FTA → USDT: use sellFta on Core
 const rawIn = ethers.parseUnits(netInput.toString(), this.ftaDecimals);
 const rawOut = await this.core.sellFta(rawIn);
 netOutput = parseFloat(ethers.formatUnits(rawOut, this.usdtDecimals));
 }
 ...
 const fromT = isUsdtTo ? 'USDT' : 'FTA';
 const toT   = isUsdtTo ? 'FTA' : 'USDT';

 document.getElementById('swap-detail-rate').innerText = isUsdtTo
 ? `1 USDT = ${this.ftaPriceUsd > 0 ? (1 / this.ftaPriceUsd).toFixed(2) : '?'} FTA`
 ...
 console.warn('calcSwap error', e);
 document.getElementById('swap-to-in').value = '0';
 }
 }

 async executeSwap(){
 const valStr = document.getElementById('swap-from-in').value;
 if (!valStr || parseFloat(valStr) <= 0) return this.showToast(this.t('invalidAmount'), true);

 this.setLoader(true, this.t('swapping'));

 const isUsdtTo = this.swapDirection === 'USDT_TO_FTA';
 const decimals = isUsdtTo ? this.usdtDecimals : this.ftaDecimals;
 const amount = ethers.parseUnits(valStr, decimals);

 // Calculate minOut with slippage
 const fee = parseFloat(valStr) * SWAP_FEE_RATE;
 const netInput = parseFloat(valStr) - fee;
 let expectedOut;

 try {
 if (isUsdtTo) {
 expectedOut = parseFloat(ethers.formatUnits(
 await this.core.buyFta(ethers.parseUnits(netInput.toString(), this.usdtDecimals)),
 this.ftaDecimals
 ));
 } else {
 expectedOut = parseFloat(ethers.formatUnits(
 await this.core.sellFta(ethers.parseUnits(netInput.toString(), this.ftaDecimals)),
 this.usdtDecimals
 ));
 }
 } catch(e) {
 ...
 if (isUsdtTo) {
 // Check internal USDT balance
 const need = amount;
 if (this.internalU < need) {
 const have = parseFloat(ethers.formatUnits(this.internalU, this.usdtDecimals));
 this.showToast(`${this.t('errNoBal')}\nNeed: ${parseFloat(valStr).toFixed(2)} USDT\nHave: ${have.toFixed(2)} USDT`, true);
 this.setLoader(false);
 return;
 }
 tx = await this.core.swapUForF(amount, minOut, deadline);
 } else {
 // Check internal FTA balance + netFta
 if (amount > this.netFtaSold) {
 ...
 if (this.internalF < amount) {
 const have = parseFloat(ethers.formatUnits(this.internalF, this.ftaDecimals));
 this.showToast(`${this.t('errNoBal')}\nNeed: ${parseFloat(valStr).toFixed(4)} FTA\nHave: ${have.toFixed(4)} FTA`, true);
 this.setLoader(false);
 return;
 }
 ...
 const received = expectedOut;
 this.recordTransaction('swap',
 isUsdtTo ? 'USDT' : 'FTA',
 isUsdtTo ? 'FTA' : 'USDT',
 parseFloat(valStr),
 received,
 tx.hash,
 { direction: isUsdtTo ? 'USDT→FTA' : 'FTA→USDT' }
 );

 this.showToast(this.t('swapSuccess') + '\n🔗 polygonscan.com/tx/' + tx.hash.slice(0,10) + '...');
 document.getElementById('swap-from-in').value = '';
 document.getElementById('swap-to-in').value = '';
 document.getElementById('swap-details').classList.add('hidden');
 await this.refreshInternalBalances();
 ...
 if (em.includes('insufficient') || em.includes('slip') || em.includes('am0'))
 this.showToast(this.t('errSwapRejected'), true);
 else if (em.includes('ftaliq') || em.includes('noliq'))
 this.showToast(this.t('errNoFtaLiquidity'), true);
 ...
 async disconnect() {
 this.stopMiningCounter();
 if (this._dataInterval) { clearInterval(this._dataInterval); this._dataInterval = null; }
 ...
 document.getElementById('addr-display').innerText = '0x...';
 ...
 document.getElementById('viz-status').style.color = '#666';
 // Reset internal balance display
 const ubi = document.getElementById('bal-usdt-internal');
 const fbi = document.getElementById('bal-fta-internal');
 if (ubi) ubi.innerText = '0.00';
 if (fbi) fbi.innerText = '0.0000';
 this.userMachines = [];
 this.userBatteries = [];
 this.pendingBalance = 0;
 this.currentRealPower = 0;
 ...
 let ct, dc;
 if (this.sendTokenSymbol === 'USDT') { ct = this.usdt; dc = this.usdtDecimals; }
 if (this.sendTokenSymbol === 'FTA')  { ct = this.fta;  dc = this.ftaDecimals; }
 tx = await ct.transfer(to, ethers.parseUnits(amt, dc));
 }
 await tx.wait();
 this.showToast(this.t('sentSuccess'));
 this.recordTransaction('send', this.sendTokenSymbol, '', parseFloat(amt), '', tx.hash, { recipient: to });
 this.closeModals();
 await this.updateData();
 } catch(e) { this.showError(e); }
 this.setLoader(false);
 ...
 const typeIcons = { swap:'💱', buy_machine:'⛏️', buy_battery:'🔋', claim:'🎁', send:'📤', receive:'📥', plug_in:'⚡', deposit:'💰', withdraw:'💸', approve:'🔐', referral:'👥' };
 const typeLabels = { swap:'Swap', buy_machine:'Machine', buy_battery:'Battery', claim:'Claim', send:'Send', receive:'Receive', plug_in:'Plug In', deposit:'Deposit', withdraw:'Withdraw', approve:'Approve', referral:'Referral' };
 ...
 b.targetHeight = (this.vizContext.canvas.height * level) * (0.6 + Math.random() * 0.4);
 });
 }
 animateVisualizer(){
 ...
 const ie = (e?.info?.error?.message || '').toLowerCase();
 const c = es + ' ' + ie;
 ...
 if (c.includes('insufficient liquidity') || c.includes('insufficient fta liquidity') || c.includes('insufficient usdt liquidity') || c.includes('noliq') || c.includes('ftaliq')) return this.t('errLowLiquidity');
 if (c.includes('insufficient funds') || c.includes('insufficient balance') || c.includes('not enough') || c.includes('underpriced') || c.includes('exceeds allowance') || c.includes('erc20: insufficient') || c.includes('transfer amount exceeds') || c.includes('insf')) return this.t('errInsufficientFunds');
 if (c.includes('nonce') || c.includes('already known') || c.includes('replacement fee too low')) return this.t('errNonce');
 if (c.includes('already pending') || c.includes('pending transaction')) return this.t('errAlreadyPending');
 if (c.includes('timeout') || c.includes('timed out') || c.includes('deadline') || c.includes('expired')) return this.t('errTimeout');
 ...
 if (c.includes('invalid amount') || c.includes('am0')) return this.t('errSwapRejected');
 if (c.includes('revert') || c.includes('execution reverted') || c.includes('vm execution error') || c.includes('gas required exceeds allowance') || c.includes('transaction failed') || c.includes('badtype') || c.includes('nom') || c.includes('nobat')) return this.t('errContract');
 ...
 const c = document.getElementById('active-machines-list');
 if (!c) return;
 const now = Math.floor(Date.now() / 1000);
 const active = this.userMachines.filter(m => m.expiresAt > now);
 if (!active.length) {
 c.innerHTML = `<p class="small-text" style="text-align:center;">${this.t('noActiveMachines')}</p>`;
 return;
 }
 const tn = ['MK-I','MK-II','MK-III','MK-IV','MK-V','MK-VI','MK-VII','MK-VIII'];
 c.innerHTML = active.map(m => {
 // Find which battery was used (derived from expiry time)
 const rem = m.expiresAt - now;
 let dur = 30; // default
 // Try to guess battery duration from remaining + elapsed
 for (const [tid, bdur] of Object.entries(this.batteryTypeDurations)) {
 const expectedExp = now + rem;
 if (bdur > 0) {
 dur = bdur;
 break;
 }
 ...
 renderUserMachines(){
 const c = document.getElementById('my-machines-list');
 if (!c) return;
 if (!this.userMachines.length) {
 c.innerHTML = `<p class="small-text" style="text-align:center;">${this.t('noMachines')}</p>`;
 return;
 }
 const now = Math.floor(Date.now() / 1000);
 const tn = ['MK-I','MK-II','MK-III','MK-IV','MK-V','MK-VI','MK-VII','MK-VIII'];
 ...
 const rem = m.expiresAt - now;
 let dur = 30;
 for (const [, bdur] of Object.entries(this.batteryTypeDurations)) { if (bdur > 0) { dur = bdur; break; } }
 const tot = dur * 86400;
 const el = tot - rem;
 const pr = Math.min(Math.max((el / tot) * 100, 0), 100);
 const bc = rem <= 0 ? 'red' : (pr < 60 ? 'green' : (pr < 85 ? 'yellow' : 'red'));
 ...
 renderUserBatteries(){
 const c = document.getElementById('my-batteries-list');
 if (!c) return;
 const types = Object.entries(this.userBatteries).filter(([,cnt]) => cnt > 0);
 if (!types.length) {
 c.innerHTML = `<p class="small-text" style="text-align:center;">${this.t('noBatteries')}</p>`;
 return;
 }
 c.innerHTML = types.map(([tid, cnt]) => {
 const dur = this.getBatteryDuration(Number(tid));
 const cl = Math.floor(Math.random() * 40) + 60;
 ...
 getMachineSVG(tier){
 const t = [
 { n:'MK-I',    g:1, c:'#64748b', a:'#94a3b8', f:1 },
 { n:'MK-II',   g:2, c:'#3b82f6', a:'#60a5fa', f:1 },
 { n:'MK-III',  g:3, c:'#8b5cf6', a:'#a78bfa', f:2 },
 { n:'MK-IV',   g:4, c:'#F0B90B', a:'#FFD43B', f:2 },
 { n:'MK-V',    g:5, c:'#f97316', a:'#fb923c', f:2 },
 { n:'MK-VI',   g:6, c:'#ef4444', a:'#f87171', f:3 },
 { n:'MK-VII',  g:8, c:'#06b6d4', a:'#22d3ee', f:3 },
 { n:'MK-VIII', g:8, c:'#eab308', a:'#facc15', f:4 }
 ][tier % 8];
 const W = 260, H = 170;
 let gH = '', fH = '', lH = '', vH = '';
 const gw = 24, gh = 48, gG = 3, mW = W - 40;
 let eg = gw;
 ...
 if (fy + 2 < gY + gh - 10) {
 for (let f = 0; f < fC; f++) {
 gH += `<rect x="${x + 3 + f * (fS / fC)}" y="${fy}" width="${Math.max(1, (fS / fC) - 1.5)}" height="2" rx="0.5" fill="${t.a}" opacity="0.12"/>`;
 }
 }
 }
 const cW = Math.min(10, eg - 6);
 gH += `<rect x="${x + (eg - cW) / 2}" y="${gY + gh - 11}" width="${cW}" height="7" rx="1.5" fill="${t.c}" opacity="0.35"/><circle cx="${x + eg / 2}" cy="${gY + 3}" r="1" fill="${t.a}" class="led-pulse" style="animation-delay:${i * 0.3}s"/>`;
 }
 const fR = 14, fS2 = 14, tFW = t.f * fR * 2 + (t.f - 1) * fS2, fSX = (W - tFW) / 2, fY = 100;
 for (let i = 0; i < t.f; i++) {
 ...
 for (let i = 0; i < 6; i++) {
 lH += `<circle cx="${25 + i * 9}" cy="148" r="1.8" fill="${i === 0 ? '#10b981' : (i < 3 ? t.a : '#334155')}" class="led-pulse" style="animation-delay:${i * 0.4}s"/>`;
 }
 ...
 const bc = [
 'background:#64748b;color:#fff','background:#3b82f6;color:#fff','background:#8b5cf6;color:#fff',
 'background:#F0B90B;color:#000','background:#f97316;color:#fff','background:#ef4444;color:#fff',
 'background:#06b6d4;color:#000','background:#eab308;color:#000'
 ];
 ...
 if (a && !this.chatInitialized) {
 this.chatInitialized = true;
 setTimeout(() => this.addChatBubble('assistant', this.getWelcomeMessage()), 400);
 }
 if (a) setTimeout(() => document.getElementById('chat-input').focus(), 350);
 }
 sendChatMessage(){
 ...
 if (!m) return;
 i.value = '';
 this.addChatBubble('user', m);
 this.chatHistory.push({ role: 'user', text: m });
 const tid = this.showTyping();
 setTimeout(() => {
 this.removeTyping(tid);
 const r = this.generateLocalResponse(m);
 this.addChatBubble('assistant', r);
 this.chatHistory.push({ role: 'assistant', text: r });
 }, 400 + Math.min(m.length * 25, 1200) + Math.random() * 400);
 }
 ...
 t.id = id;
 t.className = 'chat-bubble assistant';
 ...
 getWelcomeMessage(){
 const liqInfo = this.netFtaSold === 0n
 ? `\n⚠️ Protocol liquidity is building. Deposit USDT first, then swap USDT→FTA!`
 : `\n💧 Protocol Liquidity: ${parseFloat(ethers.formatUnits(this.netFtaSold, this.ftaDecimals)).toFixed(2)} FTA`;
 const msgs = {
 en: `👋 Welcome to Fitia Pro! I'm your AI assistant.\n\n🏗️ 4 revolutionary visions: Mining • Finance • Shop • Store\n💰 NEW: Deposit tokens to Core first, then buy machines & swap!\n💬 Ask me anything about mining, swaps, deposits, or getting started!${liqInfo}`
 };
 ...
 liquidity: this.netFtaSold === 0n ? '0' : parseFloat(ethers.formatUnits(this.netFtaSold, this.ftaDecimals)).toFixed(2) + ' FTA',
 power: this.formatHashrate(this.currentRealPower),
 active: this.userMachines.filter(m => m.expiresAt > Math.floor(Date.now()/1000)).length,
 pending: this.pendingBalance.toFixed(5),
 internalU: parseFloat(ethers.formatUnits(this.internalU, this.usdtDecimals)).toFixed(2),
 internalF: parseFloat(ethers.formatUnits(this.internalF, this.ftaDecimals)).toFixed(4)
 };
 if (intent === 'greeting' && this.user) return `👋 Hello! Connected. ${ctx.active} active machine(s) at ${ctx.power}.\n💰 Internal: ${ctx.internalU} USDT | ${ctx.internalF} FTA\nHow can I help?`;
 if (intent === 'greeting') return `👋 Welcome! Connect your wallet to start mining!`;
 if (intent === 'price' && this.user) return `📊 FTA: $${ctx.ftaPrice} | Liquidity: ${ctx.liquidity} | Pending: ${ctx.pending} FTA\n💰 Internal: ${ctx.internalU} USDT | ${ctx.internalF} FTA`;
 if (intent === 'fta_problems' && this.user) return `⚠️ Liquidity: ${ctx.liquidity}\n💰 Your internal: ${ctx.internalU} USDT | ${ctx.internalF} FTA\n\n💡 Deposit USDT to Core first (Wallet tab), then use USDT for purchases & swaps.`;
 return this.pickResponse(intent);
 }
 pickResponse(intent){
 const L = this.currentLang;
 const resp = CHAT_RESPONSES[intent] || CHAT_RESPONSES['default'];
